...
Dataset_Name <- subset(Dataset_Name, Dataset_Name$HEALTHOUTCOMEID==1 & Dataset_Name$STATE==06)
...
Hosp_dup <- cbind(Dataset_Name$STATE, Dataset_Name$HEALTHOUTCOMEID, Dataset_Name$COUNTY, Dataset_Name$YEARADMITTED, Dataset_Name$ADMISSIONMONTH, Dataset_Name$AGEGROUP, Dataset_Name$SEX)
...
Check_4 <- as.matrix(with(Dataset_Name, MONTHLYHOSP%%2==0))

#Summing provides the total of the evens and summing all zeros provides the total of the odds.
...
Check_8 <- as.matrix(CrossTable(Dataset_Name$AGEGROUP, Dataset_Name$YEARADMITTED, digits = 3, prop.r = FALSE, prop.c = TRUE, prop.t = FALSE, prop.chisq = FALSE))

# Frequency and Percent Records by Gender

Check_9 <- as.matrix(CrossTable(Dataset_Name$SEX, Dataset_Name$YEARADMITTED, digits=3, prop.r = FALSE, prop.c=TRUE, prop.t=FALSE, prop.chisq = FALSE))
...
Check_10 <- Dataset_Name %>% group_by(YEARADMITTED) %>% summarise(sum(MONTHLYHOSP))

# Sum of events by County/year

Check_11 <- Dataset_Name %>% group_by(YEARADMITTED, COUNTY) %>% summarise(sum(MONTHLYHOSP))

# Sum of events by County/Month/Year

Check_12 <- Dataset_Name %>% group_by(YEARADMITTED, ADMISSIONMONTH, COUNTY) %>% summarise(sum(MONTHLYHOSP))

# Sum of events by age group

Check_13 <- Dataset_Name %>% group_by(YEARADMITTED, AGEGROUP) %>% summarise(sum(MONTHLYHOSP))

# Sum of Events by Gender/Year

Check_14 <- Dataset_Name %>% group_by(YEARADMITTED, SEX) %>% summarise(sum(MONTHLYHOSP))
